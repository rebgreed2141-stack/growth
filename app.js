...
const SERVER_URL_STORAGE_KEY = "growth_server_url";
...
const serverUrlSaveBtn = el("serverUrlSaveBtn");
const serverSendBtn = el("serverSendBtn");
...
function saveServerUrlFromInput() {
  const url = normalizeServerUrl(serverUrlInput ? serverUrlInput.value : "");
  localStorage.setItem(SERVER_URL_STORAGE_KEY, url);
  if (serverUrlInput) serverUrlInput.value = url;
  setStatus("サーバーURLを保存しました。", manageStatus);
  showToast("保存しました");
}
...
function countGrowthRecords(records) {
  return Object.values(records || {}).reduce((total, bucket) => {
    if (!bucket || typeof bucket !== "object") return total;
    return total + Object.values(bucket).filter(Boolean).length;
  }, 0);
}

function collectGrowthJsonPayload() {
  return {
    app: "growth",
...
async function sendGrowthToServer() {
  try {
    const payload = collectGrowthJsonPayload();
    const recordCount = countGrowthRecords(payload.records);

    if (!recordCount) {
      setStatus("送信するデータがありません。", manageStatus);
      alert("送信するデータがありません");
      return;
    }

    const response = await fetch(buildApiUrl("/api/growth"), {
      method: "POST",
    ...
    const count = result.count || recordCount;
    setStatus(`サーバーに送信しました。${count}件`, manageStatus);
    alert(`サーバーに送信しました\n${count}件`);
  } catch (error) {
    console.error(error);
    setStatus("サーバーに送信できませんでした。", manageStatus);
...
function bindEvents() {
  ...
  if (serverSendBtn) {
    serverSendBtn.addEventListener("click", sendGrowthToServer);
  }
